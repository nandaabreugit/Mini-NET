...
class ClienteAGUI:
    def __init__(self):
        self.janela = tk.Tk()
        self.janela.title("Cliente A - Mini-NET Chat")
    ...
    def criar_interface(self):
        ...
        self.text_chat.tag_config("other", foreground='#4caf50')
        ...
        self.adicionar_mensagem_sistema("Cliente A iniciado. Conectando ao servidor...")
    
    def iniciar_cliente(self):
        try:
            self.cliente = ClienteBase(
                vip=config.VIP_CLIENTE_A,
                mac=config.MAC_CLIENTE_A,
                porta=config.CLIENTE_A_PORTA,
                nome="Cliente_A",
                gui=self
            )
    ...
    def enviar_mensagem(self, event=None):
        ...
        if destino == "Cliente B (Privado)":
            mensagem_formatada = f"[Privado para Cliente B] {mensagem}"
        else:
